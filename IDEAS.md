...
- Fear and Greed index (COS-25 on the board): a strip entry and a hub row. Alternative.me's `/fng/` endpoint is keyless and returns the value, the label ("Extreme Fear" … "Extreme Greed") and a history; a provider with one instrument, or a pseudo-symbol the strip understands. Folded in from the Windows dock idea, 2026-09-06.
