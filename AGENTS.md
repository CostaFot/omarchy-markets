...
The data core with two keyless providers, plus the QML: the bar strip and a multi-page panel (hub, search, watchlist, favorites, detail with membership).
...
Store.qml                    QtObject: runs bin/markets via Process, holds the snapshot, poll Timer, extras, theme colours
