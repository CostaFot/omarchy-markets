...
// The Markets pages: a hub that funnels into Search, Watchlist, Favorites
// and Portfolio, and a detail page per instrument where membership and the
// holding are managed. Pages live on a stack; Escape and Backspace walk
// back, Escape on the hub asks the host to close. One component, shown by
// two hosts: the bar popup (Panel.qml, the kit's KeyboardPanel under the
// strip) and the window (Window.qml, a FloatingWindow that Hyprland tiles
// or floats like any app). This owns the Store, so the strip in the bar
// and the rows in the popup are the same document; the window runs a Store
// of its own on the same settings (the helper's cache makes the second
// poll a read).
//
...
//
// The host sets `settings` (the bar widget's shell.json entry), `shell`,
// `hostWidget` (the bar widget whose entry holds the settings, when one is
// placed), `opened`, `inPopup` (the hub's Open as a window row) and the
// colours, and answers `closeRequested`, `openRequested` (a page asked for
// while closed) and `switchRequested` (Tab: the popup switches bar panels,
// the window does nothing). `desiredHeight` and `focusTarget` are for the
// host's sizing and focus.
FocusScope {
  id: root

  readonly property string moduleName: "costafot.markets"
  ...
  // ---- Settings -----------------------------------------------------------
  // The seven scalar settings live inline on the bar widget's shell.json entry. The page
  ...
  readonly property var settingsDefaults: ({ strip: "favorites", stripShowPrice: true, stripMax: 6, refreshMinutes: 10, portfolioCurrency: "USD", showRateLimitErrors: true, openAsWindow: false })
  ...
  function loadPendingSettings() {
    ...
    pendingOpenAsWindow = settingValue("openAsWindow") === true
    formCursor = 0
  ...
  function persistSettings(values) {
    ...
    // The bar widget's entry is the source: assigning it re-injects the
    // popup's copy and the window's binding follows. Without a widget the
    // copy here is all there is (session only).
  ...
  function saveSettings() {
    ...
    var next = { strip: pendingStrip, stripShowPrice: pendingStripShowPrice, stripMax: pendingStripMax,
                 ...
                 showRateLimitErrors: pendingShowRateLimitErrors, openAsWindow: pendingOpenAsWindow }
    var changed = {}
    var any = false
  ...
  readonly property var formControls: [stripDropdown, showPriceToggle, stripMaxField, refreshField,
                                       currencyDropdown, rateLimitToggle, openAsWindowToggle, saveButton, cancelButton]
  ...
  function stepFormControl(dx) {
    ...
    else if (c === openAsWindowToggle) pendingOpenAsWindow = !pendingOpenAsWindow
  }
  ...
  function hubRows() {
    ...
    // nf-fa-window_maximize, an escape (see the note on glyphs in AGENTS.md).
    if (inPopup)
  ...
  // The hub's Open as a window row (the popup alone): the popup closes and
  // the window opens on the hub through the shell's summon, which the
  // window root (Window.qml) answers.
  function openAsWindowRow() {
    closeRequested()
    if (shell && typeof shell.summon === "function") shell.summon(moduleName, JSON.stringify({ page: "hub" }))
  }
  ...
  // For the host: the popup fits this under its cap, the window scrolls.
  readonly property real desiredHeight: isHoldingForm ? holdingForm.implicitHeight + Style.space(12)
    : isSettingsForm ? settingsForm.implicitHeight + Style.space(12)
    : contentColumn.implicitHeight + (hasField ? filterField.height + Style.space(6) : 0)
  readonly property Item focusTarget: isHoldingForm ? qtyField : (hasField ? filterField : keyCatcher)
        ...
        Flickable {
          ...
          Column {
            ...
            Toggle {
              ...
              description: "Left click and the open, toggle and page verbs open the window instead of the popup; right click opens the popup then."
              ...
              onClicked: { root.formCursor = 6; root.pendingOpenAsWindow = !root.pendingOpenAsWindow }
            }
