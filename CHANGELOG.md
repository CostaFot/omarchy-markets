...
- Added: the same pages open as a normal window. `omarchy-shell costafot.markets window` takes `open`, `close`, `toggle` or a page name (`window portfolio` opens it on that page, or moves an open one there), and the shell's generic `omarchy-shell shell summon costafot.markets '{"page":"portfolio"}'` does the same. It is a Wayland toplevel (class `org.quickshell`, title `Markets`) that Hyprland tiles or floats like any app, resizable, with ordinary keyboard focus: the same keys as the popup, Escape on the hub closes it (so does closing the window), and Tab does nothing there (in the popup it switches bar panels). It does not replace the bar popup, which stays the one-key surface by default. The README has a keybinding line and a window rule that floats it at the popup's size.
- Added: the *Open as a window* setting (`openAsWindow`, off by default), on the Settings page and through `omarchy bar set`. With it on, the bar glyph's left click and the `open`, `close`, `toggle` and `page` verbs open the window instead of the popup, so one keybinding serves whichever you prefer. Right click on the glyph opens the other surface (the window with the setting off, the popup with it on); middle click keeps refreshing. The `window` verb stays explicit either way. A hub row, *Open as a window*, at the foot of the popup's page list: Enter closes the popup and opens the window on the hub.
- Changed: the pages live once. `Panel.qml` is the popup wrapper alone (the kit's Panel and KeyboardPanel around the content, sized under the old cap), `Pages.qml` is the content both hosts show (the page stack, the rows, the forms, the keys, the Store) and `Window.qml` is the window. The manifest gains the `panel` kind with `Window.qml` as its entry point, kept loaded and hidden until asked for.
- Changed: the plugin's IPC target moved from the bar widget (one per monitor, only one of which could claim it) to the window root (one per shell). The popup verbs now go through the bar's own summon, which picks the widget on the focused monitor (what the shell used to do for them; with a `panel` kind the shell's own summon and hide address the window instead). Every verb answers a word (`opened`, `closed`, `no bar widget placed`) instead of nothing, and `status` gains `open_as_window` and `window: {opened, page, size}`.
